...
# -------------------------------------------------------------------------
# -------------------------------------------------------------------------
red_bok <- readxl::read_excel("data/ppendemic_data.xlsx") |>
...
red_bok |>  dim()
red_bok |>  names()
redbook_species <- red_bok$species_name |>  unique()
redbook_species |>  length()
tnrs_redbook <- TNRS::TNRS(redbook_species,
                           sources = "wcvp")
...
redbook_2 <- redbook_tnrs |>
  ...
  select(-all_of(c("name_matched",
                   ...
                   "taxonomic_status"    )))
redbook_species_2 <- redbook_2$species_name |>  unique()
length(redbook_species_2)
tnrs_redbook <- TNRS::TNRS(redbook_species_2,
                           sources = "wcvp")
redbook_2 |>  dim()
redbook_tnrs_2 <- redbook_2 |>
  left_join(tnrs_redbook |>
              as_tibble() |>
              select(Name_submitted, Name_matched,
                     Accepted_family, Accepted_name, Accepted_name_author,
...
bind_rows(
redbook_tnrs |> mutate(fuente = "redbook_original"),
redbook_tnrs_2 |> mutate(fuente = "non_accepted_names")
)

redbook_tnrs |>
  filter(str_detect(accepted_name,"Lantana x armata") ) |>
  select(species_name, accepted_name, taxonomic_status)
writexl::write_xlsx("data/tnrs_read_book_base.xlsx")

df <- readxl::read_excel("data/tnrs_read_book_base_2.xlsx")
...
ppendemic_names  <- ppendemic_data_names
ppendemic_names |>
  save(file = "data/ppendemic_names.rda")

ppendemic_names |>
summarise(n_accepted_name = n_distinct(accepted_name),
